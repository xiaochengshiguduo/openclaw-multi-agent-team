...
现有英文文件保持当前路径。中文镜像文件放在英文文件旁边，并使用 `.md` 后缀。

示例：

| 英文源文件 | 中文镜像 |
|---|---|
| `roles/main/AGENTS.md` | `roles/main/AGENTS.md` |
| `workspace-template/TEAM.md` | `workspace-template/TEAM.md` |
| `task-templates/_template/status.md` | `task-templates/_template/status.md` |
...
```json
{
  ...
  "sources": {
    ...
    "zh-CN": "workspace-template/TEAM.md"
  },
  "target": "workspace/TEAM.md",
  "strategy": "managed-overwrite",
  "kind": "workspace"
...
- manifest 中列出的每个本地化运行时源文件都有英文文件和 `.md` 镜像。
- 每个安装托管运行时 Markdown 的 manifest 项都能解析 `en` 和 `zh-CN` 两种源。
- `en` dry-run plan 只选择英文路径。
- `zh-CN` dry-run plan 在需要本地化运行时文件时只选择 `.md` 路径。
...
2. **中文运行时镜像** — 为运行时协议和任务模板新增 `.md` 镜像，保持协议含义和安全约束等价。
