...
// dry-run must not write managed target files.
{
  ...
  try {
    ...
    assert(r.stdout.includes('language: zh-CN'), 'dry-run output missing language');
    ...
    assert(plan.actions.some((a) => a.source.endsWith('.md')), 'dry-run plan did not select zh-CN manifest sources');
...
// apply should create/update allowed files and restart by default when a mock restart succeeds.
{
  ...
  try {
    ...
    assert(state.language === 'zh-CN', 'default apply did not record language');
  } finally { cleanTarget(target); }
}
...
function writeLocalizationManifestFixture(tmpRepo) {
  const updatesDir = path.join(tmpRepo, 'updates', 'runtime');
  const fixtureDir = path.join(tmpRepo, 'updates', 'runtime-fixtures');
  fs.mkdirSync(updatesDir, { recursive: true });
  fs.mkdirSync(fixtureDir, { recursive: true });
  const sourcePath = 'updates/runtime-fixtures/team.md';
  fs.writeFileSync(path.join(tmpRepo, sourcePath), '# Team\n');
  const file = {
    source: sourcePath,
...
// legacy updater state should use default zh-CN, and
// apply should record the resolved default language in state and plan helpers.
{
  ...
  try {
    ...
    const team = fs.readFileSync(path.join(target, 'workspace', 'TEAM.md'), 'utf8');
    assert(team.includes('# Team'), 'legacy state without language did not use Chinese source');
    assert(team.includes('<!-- source: ' + fx.sourcePath + ' -->'), 'managed header did not record resolved Chinese source');
    const plan = readJson(path.join(target, 'state', 'openclaw-multi-agent-team', 'last-plan.json'));
    ...
    assert(state.files['workspace/TEAM.md'].source === fx.sourcePath, 'state did not record resolved Chinese source');
  } finally { cleanTarget(target); fs.rmSync(tmpRepo, { recursive: true, force: true }); }
}
