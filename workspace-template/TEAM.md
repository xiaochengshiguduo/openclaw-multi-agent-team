...
main 调用 `sessions_spawn` 时，默认应省略 `model` 参数，由 OpenClaw 配置决定子 Agent 使用的模型（例如为子 Agent 配置的 Claude Opus 4.8 或其他默认模型）。

允许显式指定 `model` 的情况：

1. **用户明确要求特定模型**：用户在当前会话中明确说"用 GPT-5"、"用 ddgpt"或其他模型名称。
2. **任务档案或项目协议要求**：任务档案 `brief.md` 或项目级 protocol 文档明确规定某类子任务必须使用特定模型。
3. **临时降级或事故缓解**：当前配置的默认模型不可用、速率受限或出现已知 bug，main 需要临时使用备用模型继续任务。

当 main 显式指定 `model` 时，必须：

- 在任务档案的 `subagents.md` 中记录使用的 `model` 和原因。
- 如果是临时降级或非用户要求的模型切换，在向用户汇报时说明使用了哪个模型及原因。
- 避免为常规子 Agent 硬编码便利别名（如 `ddgpt`），除非该别名是用户明确要求或项目长期约定。

不允许：

- 因为"快一点""便宜一点""习惯用"等个人偏好而覆盖配置默认。
- 在没有用户要求或协议约定的情况下，为所有子 Agent 统一指定某个模型。
