...
- When `main` uses sub-agents across turns, runtime events, compaction, or `sessions_yield`, follow the subagent depth architecture in `TEAM.md`: record taskNames, depth, waiting state, cleanup policy, result lookup steps, and archived outputs before cleanup.
